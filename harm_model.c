...
void init_model(char *args[])
{
	...
	set_units(args[3]);
        ...
        #if (EMISSIVITY == KAPPA)
        Nlow = 4.*M_PI*tgamma(KAPPAVAL-4./3.)/(pow(3., 7./3.)*tgamma(KAPPAVAL-2.));
        Nhigh = (1./4.)*pow(3., (KAPPAVAL-1.)/2.)*(KAPPAVAL-2.)*(KAPPAVAL-1.)
		*tgamma(KAPPAVAL/4.-1./3.)*tgamma(KAPPAVAL/4.+4./3.);
        #endif /* EMISSIVITY */
